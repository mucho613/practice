...
#define CONVERT_0BGR_TO_0RGB(crColor) (0b00001000 |	(crColor & 0x0800) >> 9 |	(crColor & 0x0080) >> 6 |	(crColor & 0x0008) >> 3)
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_PAINT:
		...
		for(int i=0; i<SCREEN_X * SCREEN_Y; i++) {
			crColor = GetPixel(hdc, i % SCREEN_X, i / SCREEN_Y);
			bitmap[i].Char.AsciiChar = (char)':';
		 	bitmap[i].Attributes = CONVERT_0BGR_TO_0RGB(crColor);
		}
